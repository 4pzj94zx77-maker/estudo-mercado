...
async function ensurePdfReader() {
  ...
  window.pdfjsLib.GlobalWorkerOptions.workerSrc = new URL("assets/pdfjs/pdf.worker.legacy.min.js", document.baseURI).href;
...
async function handleCadernetaUpload(event) {
  ...
  try {
    ...
    } else if (error.message === "PDF_READER_UNAVAILABLE") {
      setUploadStatus("O leitor de PDF não ficou disponível. Fecha este separador, abre novamente a aplicação e tenta outra vez.", "error");
